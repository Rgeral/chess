<script lang="ts">
	...
	import type { ChessgroundInstance, CGEvents } from '$lib/types/chess';

	export let fen: string;
	export let orientation: 'white' | 'black' = 'white';
	export let viewOnly = false;

	const dispatch = createEventDispatcher<CGEvents>();
	let container: HTMLDivElement; // container DOM element
	let cg: ChessgroundInstance | null = null; // chessground instance
	...
	function destsToMap(d: Record<string, string[]>): Map<string, string[]> {
		try {
			return new Map(Object.entries(d || {})) as Map<string, string[]>;
		} catch {
			return new Map();
		}
	}
	...
	export function setDests(dests: Record<string, string[]>) {
		...
		cg.set({
			movable: {
				free: false,
				showDests: true,
				dests: destsToMap(currentDests),
				color: turnFromFen(fen)
			}
		});
	}
