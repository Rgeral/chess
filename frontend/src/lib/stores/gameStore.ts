...
import type { Game, User, UserProfile, LastMove, GameState } from '$lib/types/chess';
...
export const gameActions = {
	...
	updateGameAfterMove: (result: any) => {
					...
					for (const row of prevBoard) {
						for (const sq of row) {
							...
							if (
								prevPiece &&
								newPiece &&
								(prevPiece.type !== newPiece.type || prevPiece.color !== newPiece.color)
							) {
								toSq = id;
							}
						}
		...
		console.log(
			'[GAMESTORE] updateGameAfterMove: result.lastMove =',
			result?.lastMove,
			'result.move =',
			result?.move,
