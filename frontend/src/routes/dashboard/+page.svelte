<script lang="ts">
	...
	$: if (
		$gameStore.currentGame &&
		$gameStore.currentGame.status === 'finished' &&
		!gameOverAlerted
	) {
...
<div class="min-h-screen bg-background p-4 relative overflow-hidden">
	...
	<div class="relative z-10 max-w-6xl mx-auto">
		...
		<div class="text-center mb-8">
			...
			{:else}
				<div
					class="mx-auto max-w-md p-6 rounded-xl bg-card-80 backdrop-blur-sm border border-border-50 shadow-2xl"
				>
					...
					<div class="flex gap-3">
						<input
							...
							class="flex-1 h-11 px-3 bg-input border border-border-50 rounded-md focus:outline-none focus:ring-2 focus:ring-primary placeholder:text-muted-foreground"
						/>
						<button
							class="h-11 px-5 rounded-md bg-primary text-white font-semibold disabled:opacity-60"
							on:click={createUser}
		...
		{#if $gameStore.user}
			{#if $gameStore.currentGame}
				...
				<div class="space-y-6">
					<div
						class="p-6 rounded-xl bg-card-80 backdrop-blur-sm border border-border-50 shadow-2xl"
					>
						<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
							<div>
								<h3 class="text-xl font-semibold text-foreground">
									♟️ vs Stockfish (Level {$gameStore.currentGame.difficulty})
								</h3>
								<div class="text-sm text-muted-foreground">
									Status: <span class="font-medium text-foreground"
										>{$gameStore.currentGame.status}</span
									>
									• Moves:
									<span class="font-medium text-foreground"
										>{$gameStore.currentGame.movesCount}</span
									>
								</div>
							</div>
							<div class="text-center">
								<div
									class="text-2xl font-bold text-primary bg-card rounded-lg px-4 py-2 inline-block"
								>
					...
					<div
						class="p-4 rounded-xl bg-card-80 backdrop-blur-sm border border-border-50 shadow-2xl"
					>
						<div class="max-w-3xl mx-auto">
							<ChessBoard
								lastMove={$gameStore.lastMove}
								allowMoves={$gameStore.currentGame.status === 'active'}
							/>
						</div>
					</div>
					...
					<div class="flex gap-3 justify-center">
						<button
							class="h-11 px-5 rounded-full bg-card-80 text-foreground border border-border-50"
							on:click={resetGame}>🏠 Back to Menu</button
						>
						<button
							class="h-11 px-5 rounded-full bg-primary text-white disabled:opacity-60"
							on:click={resignGame}
							disabled={$gameStore.currentGame.status !== 'active'}>🚩 Resign</button
						>
					</div>
				</div>
			{:else}
				...
				<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<div class="space-y-6">
						<div
							class="p-6 rounded-xl bg-card-80 backdrop-blur-sm border border-border-50 shadow-2xl text-center space-y-4"
						>
							...
							<div>
								...
								<div class="space-y-3">
									<div class="flex justify-between items-center">
										...
										<span class="px-2 py-1 rounded-md bg-secondary text-foreground font-semibold"
											>{$gameStore.userProfile?.user.totalGames ?? 0}</span
										>
									</div>
									<div class="flex justify-between items-center">
										<span class="text-muted-foreground">Taux de victoire</span>
										<span class="px-2 py-1 rounded-md bg-primary text-white font-semibold"
											>{getWinRate(
												$gameStore.userProfile?.user.gamesWon ?? 0,
												$gameStore.userProfile?.user.totalGames ?? 0
											)}%</span
										>
									</div>
									<div class="flex justify-between items-center">
										<span class="text-muted-foreground">Classement</span>
										<span class="px-2 py-1 rounded-md border border-border-50 font-semibold"
											>{$gameStore.userProfile?.user.estimatedElo ?? 800}</span
										>
									</div>
								</div>
								<div class="mt-4">
						...
						>
							<h3 class="text-lg font-semibold text-foreground mb-4 flex items-center">
								<span class="mr-2">♟</span>Parties Récentes
							</h3>
							<div class="space-y-3">
								{#each [{ opponent: 'IA Niveau 3', result: 'Victoire', moves: '32', time: '15min' }, { opponent: 'IA Niveau 2', result: 'Défaite', moves: '28', time: '12min' }, { opponent: 'IA Niveau 4', result: 'Victoire', moves: '45', time: '22min' }] as game, i}
									<div class="flex justify-between items-center p-3 rounded-lg bg-card-80">
										<div>
											...
											<div class="text-xs text-muted-foreground">
												{game.moves} coups • {game.time}
											</div>
										</div>
										<span
											class={`text-xs px-2 py-1 rounded-md ${game.result === 'Victoire' ? 'bg-primary text-white' : 'bg-card'}`}
											>{game.result}</span
										>
									</div>
								{/each}
					...
					<div class="space-y-6">
						<div
							class="p-8 rounded-xl bg-card-80 backdrop-blur-sm border border-border-50 shadow-2xl text-center space-y-6"
						>
							...
							<div class="space-y-4">
								<div class="text-center">
									<div class="text-lg font-medium text-foreground mb-2">
										Difficulté: <span class="text-primary"
											>{difficulty <= 5
												? 'Débutant'
												: difficulty <= 10
													? 'Facile'
							...
							<button
								class="w-full h-12 text-lg font-semibold rounded-full bg-primary text-white disabled:opacity-60"
						...
						<div class="grid grid-cols-2 gap-4">
							<button
								class="h-16 rounded-xl border border-border-50 bg-transparent flex flex-col items-center justify-center space-y-1"
					...
					<div class="space-y-6">
						...
						>
							<h3 class="text-lg font-semibold text-foreground mb-4 flex items-center">
								<span class="mr-2">🏅</span>Succès
							</h3>
							<div class="space-y-3">
								{#each [{ name: 'Première Victoire', icon: '🎯', unlocked: true }, { name: '10 Parties', icon: '🔥', unlocked: true }, { name: 'Maître Tactique', icon: '🧠', unlocked: false }, { name: 'Roi des Échecs', icon: '👑', unlocked: false }] as achievement}
									<div
										class={`flex items-center space-x-3 p-2 rounded-lg ${achievement.unlocked ? 'bg-card-80' : 'bg-card-80 opacity-50'}`}
									>
						...
						>
							<h3 class="text-lg font-semibold text-foreground mb-4 flex items-center">
								<span class="mr-2">📚</span>Apprentissage
							</h3>
							<div class="space-y-3">
								{#each [{ title: 'Tutoriel Débutant', subtitle: 'Apprenez les bases' }, { title: 'Tactiques Avancées', subtitle: 'Perfectionnez votre jeu' }, { title: 'Analyse de Parties', subtitle: 'Étudiez vos erreurs' }] as item}
				...
				{#if showStats && $gameStore.userProfile}
					<div
						class="mt-6 p-6 rounded-xl bg-card-80 backdrop-blur-sm border border-border-50 shadow-2xl"
					>
							...
							<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
								{#each $gameStore.userProfile.records as record}
									<div class="p-4 rounded-lg bg-card border border-border-50">
										...
										<div class="text-sm text-muted-foreground">
											⏱️ {gameActions.formatTime(record.bestTimeSeconds)}
										</div>
								...
								{#each $gameStore.userProfile.levelStats as stats}
									<div class="p-4 rounded-lg bg-card border border-border-50">
										<div class="flex items-center justify-between mb-2">
											...
											<span class="px-2 py-1 rounded-md bg-primary text-white text-xs"
												>{getWinRate(stats.gamesWon, stats.gamesPlayed)}%</span
											>
										</div>
										<div class="text-sm text-muted-foreground">
											{stats.gamesPlayed} games • {stats.gamesWon} wins
										</div>
										<div class="text-sm text-muted-foreground">
											⏱️ Avg: {gameActions.formatTime(stats.averageTimeSeconds)}
										</div>
										<div class="text-sm text-muted-foreground">
											♟️ Avg: {stats.averageMoves} moves
										</div>
									</div>
								{/each}
							</div>
					...
					>
						...
						<div>
							{#each leaderboard as player, index}
								<div
									class="py-3 flex items-center justify-between border-t border-border-50 first:border-t-0"
								>
									<div class="flex items-center gap-4">
										...
										<div>
											...
											<div class="text-xs text-muted-foreground">
												{player.gamesWon}/{player.totalGames} games {#if (player.currentStreak ?? 0) > 0}•
													🔥{player.currentStreak}{/if}
											</div>
										</div>
									</div>
		...
		{#if $gameStore.pendingPromotion?.isActive}
			<div class="fixed top-4 right-4 rounded-xl bg-primary text-white px-4 py-2 shadow-2xl">
		...
		{#if $gameStore.loading}
			<div class="mt-6 p-4 rounded-lg bg-card border border-border-50 text-center font-semibold">
				⏳ Loading...
			</div>
		{/if}
		{#if $gameStore.error}
			<div
				class="mt-6 p-4 rounded-lg bg-card border border-border-50 text-center font-semibold text-red-600"
			>
				❌ {$gameStore.error}
			</div>
		{/if}
